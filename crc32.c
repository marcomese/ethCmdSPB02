...
/**
 * crc_32() calculates the IEEE 802.3 crc32 of a block of bytes.
 *
 * @param data unsigned char pointer to the block of data
 * @param dataLen
 * @param crc crc value of the previous block of data, to be used
 *            when the whole block is divided in chuncks and the
 *            crc function must be used on them all in sequence.
 *            First block value must be startCRC32 constant (all 1s).
 * @return crc
 */
unsigned int crc_32(unsigned char *data, unsigned int dataLen, unsigned int crc)
{
